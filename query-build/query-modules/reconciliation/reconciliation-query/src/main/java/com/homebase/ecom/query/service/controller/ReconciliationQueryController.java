...
@RestController
@ChenileController(value = "reconciliationQuery", serviceName = "searchService")
public class ReconciliationQueryController extends ControllerSupport {
    @SecurityConfig(authoritiesSupplier = "queryAuthorities")
    @PostMapping({ "/reconciliation/{queryName}" })
