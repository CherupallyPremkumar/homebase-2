Feature: Tests the Storefront Query Service using a REST client.
  ...
  Scenario: Storefront product cards returns published products with pagination
    ...
    And the REST response contains key "list[0].row.price"
  ...
  Scenario: Storefront price ranges
    ...
    And the REST response contains key "list[0].row.maxPrice"

  Scenario: Storefront my addresses for a customer
    When I POST a REST request to URL "/q/my-addresses" with payload
    """
    {
      "filters": {
        "userId": "user-1"
    ...
    Then the http status code is 200
    And success is true
    And the REST response contains key "list[0].row.addressLine1"
    And the REST response contains key "list[0].row.city"

  Scenario: Storefront my returns for a customer
    When I POST a REST request to URL "/q/my-returns" with payload
    """
    {
      "filters": {
        "customerId": "cust-1"
    ...
    And the REST response contains key "list[0].row.reason"
