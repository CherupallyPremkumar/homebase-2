...
/**
 * Supplier dashboard query controller -- supplier-specific queries.
 * Reads from hm_query.* OLAP tables:
 * seller_dashboard_flat, order_details_flat, order_items_flat,
 * settlement_flat, product_catalog_flat, inventory_health_flat,
 * return_detail_flat, fulfillment_flat.
 */
@RestController
@ChenileController(value = "supplierDashboardQuery", serviceName = "searchService")
public class SupplierDashboardQueryController extends ControllerSupport {

    @SecurityConfig(authoritiesSupplier = "queryAuthorities")
    @PostMapping({ "/supplier-dashboard/{queryName}" })
