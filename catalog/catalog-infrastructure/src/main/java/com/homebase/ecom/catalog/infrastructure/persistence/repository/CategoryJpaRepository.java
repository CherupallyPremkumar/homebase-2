...
public interface CategoryJpaRepository extends JpaRepository<CategoryEntity, String> {
    @Query("SELECT c FROM CategoryEntity c WHERE c.parentId IS NULL AND c.active = true ORDER BY c.displayOrder")
    List<CategoryEntity> findRootCategories();

    @Query("SELECT c FROM CategoryEntity c WHERE c.parentId = :parentId AND c.active = true ORDER BY c.displayOrder")
