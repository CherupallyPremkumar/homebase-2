...
public interface InventoryItemJpaRepository extends JpaRepository<InventoryItemEntity, String> {
    ...
    @Query("SELECT i FROM InventoryItemEntity i JOIN i.activeReservations r WHERE r.orderId = :orderId")
    List<InventoryItemEntity> findByOrderIdInReservations(@Param("orderId") String orderId);

    @Query("SELECT COUNT(i) > 0 FROM InventoryItemEntity i JOIN i.activeReservations r WHERE r.orderId = :orderId")
