Feature: Tests the supplierproduct Workflow Service using a REST client. This is done only for the
first testcase. Supplierproduct service exists and is under test.
...
And store "$.payload.mutatedEntity.currentState.stateId" from response to "finalState"
 ...
 Scenario: Send the discontinueProduct event to the supplierproduct with comments
...
Send cc event on the supplierproduct with comments. This should fail since the mandatory activities
have not been completed.
Given that "terminalState" equals "__TERMINAL_STATE__"
And that config strategy is "supplierproductConfigProvider" with prefix "Supplierproduct"
...
Scenario: Perform mandatory activity (a1) on the  supplierproduct with comments
...
Scenario: Perform mandatory activity (a2) on the  supplierproduct with comments
...
Scenario: Perform mandatory activity (cc) on the  supplierproduct with comments
...
Scenario: Send an invalid event to supplierproduct . This will err out.
When I PATCH a REST request to URL "/supplierproduct/${id}/invalid" with payload
"""json
{
