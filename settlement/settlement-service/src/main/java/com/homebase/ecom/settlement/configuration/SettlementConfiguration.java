...
import com.homebase.ecom.settlement.service.store.SettlementEntityStore;
...
public class SettlementConfiguration {
    ...
    @Bean BeanFactoryAdapter settlementBeanFactoryAdapter() {
	...
	@Bean  STM<Settlement> settlementEntityStm(@Qualifier("settlementFlowStore") STMFlowStoreImpl stmFlowStore) throws Exception{
		STMImpl<Settlement> stm = new STMImpl<>();		
		stm.setStmFlowStore(stmFlowStore);
		return stm;
	}
	
	@Bean  STMActionsInfoProvider settlementActionsInfoProvider(@Qualifier("settlementFlowStore") STMFlowStoreImpl stmFlowStore) {
		STMActionsInfoProvider provider =  new STMActionsInfoProvider(stmFlowStore);
        WorkflowRegistry.addSTMActionsInfoProvider("settlement",provider);
        return provider;
	}
	
	@Bean EntityStore<Settlement> settlementEntityStore() {
		return new SettlementEntityStore();
	}
	
	@Bean  StateEntityServiceImpl<Settlement> _settlementStateEntityService_(
    ...
    @Bean  DefaultPostSaveHook<Settlement> settlementDefaultPostSaveHook(
    ...
    @Bean  GenericEntryAction<Settlement> settlementEntryAction(@Qualifier("settlementEntityStore") EntityStore<Settlement> entityStore,
    @Qualifier("settlementActionsInfoProvider") STMActionsInfoProvider settlementInfoProvider,
    ...
    GenericEntryAction<Settlement> entryAction =  new GenericEntryAction<Settlement>(entityStore,settlementInfoProvider,postSaveHook);
    stmFlowStore.setEntryAction(entryAction);
    return entryAction;
    }

    @Bean  DefaultAutomaticStateComputation<Settlement> settlementDefaultAutoState(
    @Qualifier("settlementTransitionActionResolver") STMTransitionActionResolver stmTransitionActionResolver,
    @Qualifier("settlementFlowStore") STMFlowStoreImpl stmFlowStore){
	...
	@Bean GenericExitAction<Settlement> settlementExitAction(@Qualifier("settlementFlowStore") STMFlowStoreImpl stmFlowStore){
	...
	@Bean SettlementHealthChecker settlementHealthChecker(){
    	return new SettlementHealthChecker();
    }

    @Bean STMTransitionAction<Settlement> defaultsettlementSTMTransitionAction() {
    ...
    @Bean  StmBodyTypeSelector settlementBodyTypeSelector(
    @Qualifier("settlementActionsInfoProvider") STMActionsInfoProvider settlementInfoProvider,
    @Qualifier("settlementTransitionActionResolver") STMTransitionActionResolver stmTransitionActionResolver) {
    ...
    @Bean  STMTransitionAction<Settlement> settlementBaseTransitionAction(
        @Qualifier("settlementTransitionActionResolver") STMTransitionActionResolver stmTransitionActionResolver,
    ...
    @Bean ActivityChecker settlementActivityChecker(@Qualifier("settlementFlowStore") STMFlowStoreImpl stmFlowStore){
        return new ActivityChecker(stmFlowStore);
    }

    @Bean
    AreActivitiesComplete activitiesCompletionCheck(@Qualifier("settlementActivityChecker") ActivityChecker activityChecker){
        return new AreActivitiesComplete(activityChecker);
    }

    // Create the specific transition actions here. Make sure that these actions are inheriting from
    // AbstractSTMTransitionMachine (The sample classes provide an example of this). To automatically wire
    // them into the STM use the convention of "settlement" + eventId + "Action" for the method name. (settlement is the
    ...
    @Bean RetrySettlementSettlementAction
            settlementRetrySettlementAction(){
        return new RetrySettlementSettlementAction();
    }

    @Bean MonthEndSettlementAction
            settlementMonthEndAction(){
        return new MonthEndSettlementAction();
    }

    @Bean RejectSettlementSettlementAction
            settlementRejectSettlementAction(){
        return new RejectSettlementSettlementAction();
    }

    @Bean HoldForReviewSettlementAction
            settlementHoldForReviewAction(){
        return new HoldForReviewSettlementAction();
    }

    @Bean CalculateSettlementSettlementAction
            settlementCalculateSettlementAction(){
        return new CalculateSettlementSettlementAction();
    }

    @Bean TransferFundsSettlementAction
            settlementTransferFundsAction(){
        return new TransferFundsSettlementAction();
    }

    @Bean ManualApproveSettlementAction
            settlementManualApproveAction(){
        return new ManualApproveSettlementAction();
    }


    @Bean ConfigProviderImpl settlementConfigProvider() {
        return new ConfigProviderImpl();
    }

    @Bean ConfigBasedEnablementStrategy settlementConfigBasedEnablementStrategy(
    ...
    @Bean MANUAL_REVIEWSettlementPostSaveHook
        settlementMANUAL_REVIEWPostSaveHook(){
            return new MANUAL_REVIEWSettlementPostSaveHook();
    }

    @Bean SETTLEDSettlementPostSaveHook
        settlementSETTLEDPostSaveHook(){
            return new SETTLEDSettlementPostSaveHook();
    }

    @Bean FAILEDSettlementPostSaveHook
        settlementFAILEDPostSaveHook(){
            return new FAILEDSettlementPostSaveHook();
    }

    @Bean PROCESSINGSettlementPostSaveHook
        settlementPROCESSINGPostSaveHook(){
            return new PROCESSINGSettlementPostSaveHook();
    }

    @Bean PENDINGSettlementPostSaveHook
        settlementPENDINGPostSaveHook(){
