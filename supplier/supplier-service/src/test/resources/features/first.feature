Feature: Tests the supplier Workflow Service using a REST client. This is done only for the
first testcase. Supplier service exists and is under test.
...
Send cc event on the supplier with comments. This should fail since the mandatory activities
have not been completed.
Given that "terminalState" equals "__TERMINAL_STATE__"
And that config strategy is "supplierConfigProvider" with prefix "Supplier"
...
Scenario: Perform mandatory activity (a1) on the  supplier with comments
...
Scenario: Perform mandatory activity (a2) on the  supplier with comments
...
Scenario: Perform mandatory activity (cc) on the  supplier with comments
...
When I PATCH a REST request to URL "/supplier/${id}/invalid" with payload
"""json
{
