...
/**
 * PostSaveHook for PENDING state (initial creation).
 * Logs shipment creation.
 */
public class PENDINGShippingPostSaveHook implements PostSaveHook<Shipping> {

    private static final Logger log = LoggerFactory.getLogger(PENDINGShippingPostSaveHook.class);

    @Override
    public void execute(State startState, State endState, Shipping shipping, TransientMap map) {
        log.info("Shipment {} created for order {}. State: PENDING",
