...
@Component({
          ...
          <tbody>
            @for (item of items; track item.id) {
              <tr>
                ...
                <td><span class="status" [ngClass]="'status-' + (item.status ?? '').toLowerCase()">{{ item.status }}</span></td>
              </tr>
            }
  ...
  styles: [`
    ...
    .status-in_stock, .status-active { background: #d1fae5; color: #065f46; }
    .status-low_stock, .status-pending { background: #fef3c7; color: #92400e; }
...
export class InventoryListComponent implements OnInit {
  ...
  load(page: number): void {
    this.loading = true;
    this.page = page;
    this.queryService.search('inventories', { pageNum: page }).subscribe(r => {
      this.items = r.items;
