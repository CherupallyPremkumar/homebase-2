          ...
          <tbody>
            @for (item of items; track item.id) {
              <tr>
                ...
                <td>{{ item.currency }} {{ item.totalSalesAmount | number:'1.2-2' }}</td>
                <td>{{ item.currency }} {{ item.commissionAmount | number:'1.2-2' }}</td>
                <td>{{ item.currency }} {{ item.netPayoutAmount | number:'1.2-2' }}</td>
...
export class SettlementListComponent implements OnInit {
  ...
  load(page: number): void {
    this.loading = true;
    this.page = page;
    this.queryService.search('settlements', { pageNum: page }).subscribe(r => {
      this.items = r.items;
