...
@Component({
        ...
        <table class="table">
          ...
          <tbody>
            @for (item of items; track item.id) {
              <tr>
                <td><a [routerLink]="['/orders', item.id]">{{ item.id }}</a></td>
                <td>{{ item.userId }}</td>
                <td>{{ item.totalAmount?.currency ?? 'INR' }} {{ item.totalAmount?.amount | number:'1.2-2' }}</td>
                <td><span class="status" [ngClass]="'status-' + (item.stateId ?? item.status ?? '').toLowerCase()">{{ item.stateId ?? item.status }}</span></td>
  ...
  styles: [`
    ...
    .status-active, .status-approved, .status-paid, .status-delivered { background: #d1fae5; color: #065f46; }
...
export class OrderListComponent implements OnInit {
  ...
  load(page: number): void {
    this.loading = true;
    this.page = page;
    this.queryService.search('orders', { pageNum: page }).subscribe(r => {
      this.items = r.items;
