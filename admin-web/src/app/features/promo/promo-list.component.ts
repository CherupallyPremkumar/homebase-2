...
@Component({
          ...
          <tbody>
            @for (item of items; track item.id) {
              <tr>
                ...
                <td>{{ item.discountType === 'PERCENTAGE' ? item.discountValue + '%' : (item.discountValue | number:'1.2-2') }}</td>
                <td>{{ item.expiryDate | date:'mediumDate' }}</td>
                <td>{{ item.usageCount }}{{ item.maxUsage ? ' / ' + item.maxUsage : '' }}</td>
  ...
  styles: [`
    ...
    .status-expired, .status-disabled { background: #fee2e2; color: #991b1b; }
...
export class PromoListComponent implements OnInit {
  ...
  load(page: number): void {
    this.loading = true;
    this.page = page;
    this.queryService.search('promos', { pageNum: page }).subscribe(r => {
      this.items = r.items;
