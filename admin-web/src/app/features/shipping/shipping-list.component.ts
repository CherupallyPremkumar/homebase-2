...
@Component({
        ...
        <table class="table">
          ...
          <tbody>
            @for (item of items; track item.id) {
              <tr>
                <td>{{ item.id }}</td>
                <td><a [routerLink]="['/orders', item.orderId]">{{ item.orderId }}</a></td>
                <td>{{ item.carrier }}</td>
                <td>{{ item.trackingNumber }}</td>
                <td><span class="status" [ngClass]="'status-' + (item.stateId ?? '').toLowerCase()">{{ item.stateId }}</span></td>
                <td>{{ item.shippedAt | date:'mediumDate' }}</td>
              </tr>
            }
  ...
  styles: [`
    ...
    .status-delivered, .status-shipped { background: #d1fae5; color: #065f46; }
    .status-pending, .status-in_transit { background: #fef3c7; color: #92400e; }
...
export class ShippingListComponent implements OnInit {
  ...
  load(page: number): void {
    this.loading = true;
    this.page = page;
    this.queryService.search('shippings', { pageNum: page }).subscribe(r => {
      this.items = r.items;
