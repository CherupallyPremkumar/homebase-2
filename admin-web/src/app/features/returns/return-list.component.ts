        ...
        <table class="table">
          ...
          <tbody>
            @for (item of items; track item.id) {
              <tr>
                <td><a [routerLink]="['/returns', item.id]">{{ item.id }}</a></td>
...
export class ReturnListComponent implements OnInit {
  ...
  load(page: number): void {
    this.loading = true;
    this.page = page;
    this.queryService.search('returnrequests', { pageNum: page }).subscribe(r => {
      this.items = r.items;
  ...
  triggerEvent(id: string, event: string): void {
    this.apiService.triggerEvent('returnrequests', id, event).subscribe(() => {
      this.load(this.page);
    });
  }
}
