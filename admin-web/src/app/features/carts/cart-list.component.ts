...
@Component({
          ...
          <tbody>
            @for (item of items; track item.id) {
              <tr>
                ...
                <td>{{ item.lastModifiedTime | date:'medium' }}</td>
              </tr>
            }
  ...
  styles: [`
    ...
    .status-checkout_in_progress, .status-pending { background: #fef3c7; color: #92400e; }
    .status-expired, .status-abandoned { background: #fee2e2; color: #991b1b; }
...
export class CartListComponent implements OnInit {
  ...
  load(page: number): void {
    this.loading = true;
    this.page = page;
    this.queryService.search('carts', { pageNum: page }).subscribe(r => {
      this.items = r.items;
