        ...
        <table class="table">
          ...
          <tbody>
            @for (item of items; track item.id) {
              <tr>
                <td>{{ item.id }}</td>
                <td><a [routerLink]="['/products', item.productId]">{{ item.productId }}</a></td>
                <td>{{ item.name }}</td>
                <td>{{ item.price | number:'1.2-2' }}</td>
                <td><span class="status" [ngClass]="item.featured ? 'status-active' : 'status-pending'">{{ item.featured ? 'Yes' : 'No' }}</span></td>
                <td><span class="status" [ngClass]="item.active ? 'status-active' : 'status-rejected'">{{ item.active ? 'Active' : 'Inactive' }}</span></td>
              </tr>
            }
...
export class CatalogListComponent implements OnInit {
  ...
  load(page: number): void {
    this.loading = true;
    this.page = page;
    this.queryService.search('catalogs', { pageNum: page }).subscribe(r => {
      this.items = r.items;
